...
train_data_feeder = DataFeeder(get_paths_train, preprocess_workers = 8, cuda_workers = 1, numpy_size = 20, cuda_size = 1, batch_size = 1, jitter = False, volatile = True)
val_data_feeder = DataFeeder(get_paths_val, preprocess_workers = 4, cuda_workers = 1,
                               numpy_size = 10, cuda_size = 1, batch_size = 1, jitter = False, volatile = True)
...
def test_model(images, model):
    ...
    process_draw(0.3, images, boxes, classes, use_nms = False, softmax=False, border_size=1)
