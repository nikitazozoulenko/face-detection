#cross CONV2-conv7 with bilinear interpolation upsampling, 
...
class FaceNet(nn.Module):
    def __init__(self):
        ...
        self.bottleneck_conv2 = nn.Conv2d(64*4, 256, kernel_size=1, stride=1, padding=0, bias = True)

        self.conv3 = nn.Sequential(*modules_conv3)
        self.bottleneck_conv3 = nn.Conv2d(128*4, 256, kernel_size=1, stride=1, padding=0, bias = True)
        
        self.conv4 = nn.Sequential(*modules_conv4)
        self.bottleneck_conv4 = nn.Conv2d(256*4, 256, kernel_size=1, stride=1, padding=0, bias = True)

        self.conv5 = nn.Sequential(*modules_conv5)
        self.bottleneck_conv5 = nn.Conv2d(512*4, 256, kernel_size=1, stride=1, padding=0, bias = True)

        self.conv6 = nn.Sequential(nn.Conv2d(512*4, 512*4, kernel_size=1, stride=2, padding=0, bias = True),
                                   *[ResidualBlock(128*4, expansion=4) for _ in range(2)])
  
        self.conv7 = nn.Sequential(nn.Conv2d(512*4, 512*4, kernel_size=1, stride=2, padding=0, bias = True),
                                   *[ResidualBlock(128*4, expansion=4) for _ in range(2)])
        self.bottleneck_conv6 = nn.Conv2d(512*4, 256, kernel_size=1, stride=1, padding=0, bias = True)
        self.bottleneck_conv7 = nn.Conv2d(512*4, 256, kernel_size=1, stride=1, padding=0, bias = True)

        self.prediction_head =  PredictionHead()

        self.anchors_wh2 = torch.Tensor([[8, 8],  [8, 8*1.5],
                                         [10, 10],  [10, 10*1.5],
                                         [13, 13],  [13, 13*1.5]]).cuda()
    ...
    def forward(self, x, phase = "train"):
        ...
        boxes = torch.cat((boxes2, boxes3, boxes4, boxes5, boxes6, boxes7), dim=1)
        classes =torch.cat((classes2, classes3, classes4, classes5, classes6, classes7), dim=1)
        anchors = torch.cat((anchors2, anchors3, anchors4, anchors5, anchors6, anchors7), dim=0)
