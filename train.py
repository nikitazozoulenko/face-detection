...
from network_v_2_5 import FaceNet, Loss
...
def main():
    train_data_feeder = DataFeeder(get_paths_train, preprocess_workers=4, cuda_workers=1,
                                numpy_size=14, cuda_size=4, batch_size=2, jitter=True)
    val_data_feeder = DataFeeder(get_paths_val, preprocess_workers=1, cuda_workers=1,
                                numpy_size=6, cuda_size=1, batch_size=2, jitter = False,
                                volatile=True)
